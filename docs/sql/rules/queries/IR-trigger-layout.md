...
#### ❌ Violating Example
```sql
CREATE TRIGGER update_user_access BEFORE UPDATE OF manager ON community.community_user_xref FOR EACH ROW EXECUTE FUNCTION community.community_user_manager_access_function();
```
