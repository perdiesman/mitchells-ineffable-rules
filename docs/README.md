# Mitchell's Ineffable Rules - Documentation

Welcome to the style guide and rules catalog for the Mitchell's Ineffable Rules (MIR) Linter.

## Languages

### [JAVA](java/README.md)
Categories:
- [General Rules](java/README.md#general-rules)
- [Classes and Interfaces](java/README.md#classes-and-interfaces)
- [Method Declarations](java/README.md#method-declarations)
- [Variables and Fields](java/README.md#variables-and-fields)

### [SQL](sql/README.md)
Categories:
- [General Rules](sql/README.md#general-rules)
- [Query Structure Rules](sql/README.md#query-structure-rules)
- [Schema Definition Rules](sql/README.md#schema-definition-rules)
- [Data Modification Rules](sql/README.md#data-modification-rules)
- [Routine & Procedure Rules](sql/README.md#routine-procedure-rules)

### [XML](xml/README.md)
Categories:
- [General Rules](xml/README.md#general-rules)
- [Tags and Elements](xml/README.md#tags-and-elements)
- [Attributes](xml/README.md#attributes)
- [Namespaces](xml/README.md#namespaces)

## Additional Guides
- [Custom Rules Guide](custom_rules.md)
- [PyPI Publishing Guide](publishing.md)
